class Race
    ...
    def get_group(racer)
        if racer && racer.birth_year && racer.gender
            age = date.year - racer.birth_year
            quotient = age / 10
            min_age = quotient * 10
            max_age = (quotient + 1) * 10 - 1
            gender = racer.gender
            group_name = min_age >= 60 ? "masters #{gender}" : "#{min_age} to #{max_age} (#{gender})"
            Placing.demongoize(name: group_name)
        end
    end
    ...
    def self.upcoming_available_to(racer)
        upcoming_race_ids_for_racer = racer.races.upcoming.pluck(:race).map { |race| race[:_id] }
        upcoming_race_ids = Race.upcoming.pluck(:_id)
        upcoming_available_race_ids = upcoming_race_ids - upcoming_race_ids_for_racer
        Race.in(_id: upcoming_available_race_ids)
    end
